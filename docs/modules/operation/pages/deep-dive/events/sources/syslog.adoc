...
A framing mismatch tends to show up in one of two ways.
Under `non-transparent` against an octet-counting sender, the length prefix appears at the start of the event body.
Under `octet-counting` against a newline-delimited sender, the connection is dropped with a message in `syslogd.log`.
...
Syslogd runs one TCP listener, so `tcp-tls-enabled` applies to it as a whole rather than to a second port.
...
On a Minion, TCP is a separate feature rather than an attribute.
Install `opennms-syslogd-listener-netty-tcp` and set the `syslog.tcp.*` keys in `$\{MINION_HOME}/etc/org.opennms.netmgt.syslog.cfg`.
