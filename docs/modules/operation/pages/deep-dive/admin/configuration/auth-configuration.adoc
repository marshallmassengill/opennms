...
CAUTION: The 401/403 retry path runs on the OpenNMS core only. A Minion-served collection that receives a 401 can keep sending the same cached token on subsequent cycles, until `<ttl-seconds>` elapses or a reload event flushes the cache from the core. A shorter `<ttl-seconds>` on auth definitions used through Minions reduces the window during which a stale token can keep producing failures.
