 ...
 *
 * A framing error means the position of every subsequent message in the stream is
 * unknown, so the connection cannot be recovered by skipping bytes; dropping it lets
 * the sender reconnect and resynchronise. Logs at most once per connection, because a
 * misconfigured sender that reconnects in a loop would otherwise fill the log.
 */
