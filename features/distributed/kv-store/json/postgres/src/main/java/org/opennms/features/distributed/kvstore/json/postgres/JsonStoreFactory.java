...
/**
 * Factory that creates either a {@link PostgresJsonStore} or an {@link InMemoryJsonStore}
 * depending on whether the system is running in read-only mode.
 */
public class JsonStoreFactory {

    public static JsonStore create(DataSource dataSource) {
        if (Boolean.getBoolean("opennms.readonly")) {
            return new InMemoryJsonStore();
        }
        return new PostgresJsonStore(dataSource);
    }
}
