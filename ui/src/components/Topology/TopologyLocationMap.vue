...
/**
 * No dark basemap exists to switch to: the tile providers are the operator's own
 * (usually tiles.opennms.org), OpenStreetMap and OpenTopoMap, all light. Rather
 * than reach for a third-party dark tile host, the light tiles are inverted in
 * CSS, which is the conventional Leaflet approach. hue-rotate puts the hues back
 * the right way round after the invert, so water stays blue rather than orange.
 */
