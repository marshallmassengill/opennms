 ...
 */
/**
 * The theme's topology accent, resolved to a concrete string because sigma draws
 * to a canvas and cannot use a CSS variable. Read from the document rather than
 * duplicated, so the token in onms-theme.scss stays the single definition.
 *
 * Used only for presentation -- hover and selection emphasis, recomputed every
 * frame. Colors that get *persisted* into a saved view stay theme-independent
 * (see DEFAULT_NODE_COLOR): a view built in dark mode must not save different
 * colors from one built in light mode.
 */
...
/**
 * How an unemphasized link is drawn. Applied only to links still carrying the
 * default color: a color the user picked is theirs and is left alone.
 *
 * The stored value cannot be theme-aware, since it is persisted into saved
 * views, so the theme is applied at draw time instead.
 */
...
const exportImage = async (fileName: string, format: 'png' | 'jpeg' = 'png'): Promise<void> => {
  ...
  // Match the canvas the user is looking at. Hardcoding white flattened a dark
  // view onto a white sheet while the labels kept their dark-theme color, which
  // measures 1.29:1 against white -- effectively invisible. JPEG has no alpha, so
  // it always needs an opaque color; PNG could carry transparency, but an
  // exported map is usually pasted into a document, and a transparent
  // background there shows whatever is behind it rather than the map.
