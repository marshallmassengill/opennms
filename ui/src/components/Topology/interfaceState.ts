...
/**
 * IF-MIB interface state, named rather than reduced to up/down.
 *
 * `ifOperStatus` has seven values and the difference between them is the useful
 * part: `lowerLayerDown` means the far end or a lower layer failed, where `down`
 * means this port did. Collapsing them would also mean guessing at the operator's
 * policy, since the SNMP Interface Poller's `up-values`/`down-values` are
 * per-package configuration the REST API does not expose.
 */
...
/**
 * How the state got into the database, which decides how much to trust it.
 *
 * Provisiond's node scan writes these columns on every rescan, daily by default,
 * and that is the only writer unless the SNMP Interface Poller is enabled -- it
 * is off by default and additionally needs ENABLE_POLLING per interface from a
 * provisioning policy. So the same two numbers can be seconds old or a day old,
 * and only `lastSnmpPoll` distinguishes them.
 */
