...
public abstract class IplikeSqlTranslator {
    ...
    public static String toSqlPredicate(final String pattern, final String columnExpression) {
        ...
        if (V4_MATCH_ALL.equals(pattern) || V6_MATCH_ALL.equals(pattern)) {
            // both shipped implementations fast-path the v4 form to true for
            // every non-null value; NULL never matches (iplike is strict/
     ...
     * opennms_safe_inet()'s treatment of the value (zone id stripped, strict
     * dotted-quad or 8-group format). Exists so tests can verify corpus parity
    ...
    private static String render(final int family, final BigInteger n) {
        final int len = family == 4 ? 4 : 16;
        final byte[] raw = n.toByteArray();
        final byte[] addr = new byte[len];
        final int copy = Math.min(raw.length, len);
        System.arraycopy(raw, raw.length - copy, addr, len - copy, copy);
        try {
            return InetAddress.getByAddress(addr).getHostAddress();
        } catch (final UnknownHostException e) {
            // unreachable: getByAddress only throws on illegal length
